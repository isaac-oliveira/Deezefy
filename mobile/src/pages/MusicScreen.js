...
const MusicScreen = () => {
  ...
  const renderItem = ({ item, index }) => {
    ...
    async function handleLike() {
      const response = await Api.createLike({
        email,
        musicId: item.id,
      });
      if (!response.ok) {
        Alert.alert("Opa!", "Você já curtiu");
        return;
      }
      Alert.alert("Sucesso!", "Curtido com sucesso");
    }
    return (
      <TouchableOpacity style={styles.item} onPress={handleItem}>
        ...
        <TouchableOpacity style={{ padding: 10 }} onPress={handleLike}>
